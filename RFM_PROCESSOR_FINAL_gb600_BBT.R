...
imported_data <- tibble("Wavenumber"=seq(645, 1500, 0.25),
                        "BT_1"=scan(data_file_path1, what=double(), skip=4),
                        "BT_2"=scan(data_file_path2, what=double(), skip=4))
...
data_file_path_N1 <- path_real("./bbt_nGB_600_comp_Yils_Ygrid-90000.asc")
stopifnot(file_exists(data_file_path_N1))

values_N1 <- scan(data_file_path_N1, what=double(), skip=4)
values_N1

data_file_path_N2 <- path_real("./bbt_nGB_600_comp_Yils_Ngrid-90000.asc")
stopifnot(file_exists(data_file_path_N2))

values_N2 <- scan(data_file_path_N2, what=double(), skip=4)
values_N2

# Create Tibble of data
imported_data2 <- tibble("Wavenumber"=seq(645, 1500, 0.25),
                         "BT_N1"=scan(data_file_path_N1, what=double(), skip=4),
                         "BT_N2"=scan(data_file_path_N2, what=double(), skip=4))
imported_data2

#Unify Tibbles
imported_data["BT_NoAgent1"] <- imported_data2$BT_N1
imported_data["BT_NoAgent2"] <- imported_data2$BT_N2

# Subtract each agent data set from the set with no agent
imported_data["One"] <- imported_data$BT_NoAgent1 - imported_data$BT_1
imported_data["Two"] <- imported_data$BT_NoAgent2 - imported_data$BT_2
...
# Import in Digitised Data from Leicester Report

Leic_tbl <- read.table("BBT_Leic.txt", header=FALSE, sep = ",")
...
#plot(imported_data$Wavenumber, imported_data$One, type="l", xlim=c(645, 1500), ylim=c(0, 700), main="Difference Between the Two Radiance Datasets", xlab="Wavenumber / cm-1", ylab="Radiance Difference / nW/(cm2 st cm-1)")
...
p1a <- ggplot() +
  xlim(645, 1500) +
#  scale_y_cut(breaks = 0.003, which = 1, scales = c(0.5, 1)) +
  geom_line(data = imported_data, aes(x = Wavenumber, y = One, colour ="Difference")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = Fifteen, colour ="Fifteen")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = Thirty, colour ="Thirty")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = FourtyFive, colour ="FourtyFive")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = Sixty, colour ="Sixty")) +
  #geom_line(data = IASI_Noise, aes(x = V1, y = Noise, colour = "Noise")) +
  scale_color_manual(name = "Legend", breaks = c("Difference"), values = c("Difference" = "skyblue2")) +
  labs(title = "Brightness Temperature Difference for the GB_600 Profile With a Fine Height Grid", x = "Wavenumber / cm-1", y = "Brightness Temperature Difference / K") +
  theme(text = element_text(size = 16, colour = "black"),
        plot.title = element_text(size = 12, colour = "black", hjust = 0.2),
        axis.title.x = element_text(size = 12, colour ="black"),
        axis.title.y = element_text(angle = 90, size = 14, colour ="black"),
...
p2a <- ggplot() +
  xlim(645, 1500) +
  #  scale_y_cut(breaks = 0.003, which = 1, scales = c(0.5, 1)) +
  geom_line(data = imported_data, aes(x = Wavenumber, y = Two, colour ="GB_600 Profile, No Grid")) +
  geom_line(data = Leic_tbl, aes(x = V1, y = V2, colour ="Data from Previous Study")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = Thirty, colour ="Thirty")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = FourtyFive, colour ="FourtyFive")) +
  #geom_line(data = imported_data, aes(x = Wavenumber, y = Sixty, colour ="Sixty")) +
  #geom_line(data = IASI_Noise, aes(x = V1, y = Noise, colour = "Noise")) +
  scale_color_manual(name = "Legend", breaks = c("GB_600 Profile, No Grid", "Data from Previous Study"), values = c("GB_600 Profile, No Grid" = "skyblue2", "Data from Previous Study" = "orange")) +
  labs(title = "GB_600 Profile Without a Fine Height Grid vs Sample Data from the Previous Study", x = "Wavenumber / cm-1", y = "Brightness Temperature Difference / K") +
  theme(text = element_text(size = 16, colour = "black"),
        plot.title = element_text(size = 12, colour = "black", hjust = 0.2),
        axis.title.x = element_text(size = 12, colour ="black"),
        axis.title.y = element_text(angle = 90, size = 14, colour ="black"),
