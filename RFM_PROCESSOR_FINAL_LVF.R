...
data_file_path_N <- path_real("./NO_AGENT_N-90000.asc")
...
#sort table in order of increasing wavenumber for output
imported_data3 <- imported_data %>% arrange(Wavelength)

#export csv of wavenumber and radiance data for LVF model if necessary
write.csv(imported_data3, file="LVF_Data.csv")
...
p1a <- ggplot() +
  ...
  geom_line(data = imported_data, aes(x = Wavenumber, y = One, colour ="One")) +
  ...
  #scale_color_manual(name = "Sample Time", breaks = c("One", "Fifteen", "Thirty", "FourtyFive", "Sixty", "Noise"), values = c("One" = "skyblue2", "Fifteen" = "springgreen", "Thirty" = "darkorchid4", "FourtyFive" = "magenta", "Sixty" = "orange", "Noise" = "black")) +
  scale_color_manual(name = "Sample Time", breaks = c("One", "Fifteen", "Thirty", "FourtyFive", "Sixty"), values = c("One" = "skyblue2", "Fifteen" = "springgreen", "Thirty" = "darkorchid4", "FourtyFive" = "magenta", "Sixty" = "orange")) +
  labs(title = "100 kg sulphur mustard release, individual buildings", x = "Wavenumber / cm-1", y = "Radiance Difference / nW/(cm2 sr cm-1)") +
