...
index1 <- 1
for (i in s1) {
  file_paths1[index1] <- paste0("./1/GB_B100_1J_Y-90000_gb", sprintf("%05d", i), ".asc")
  index1 <- index1+1
}
...
index2 <- 1
for (i in s2) {
  file_paths2[index2] <- paste0("./15/GB_B100_15J_Y-90000_gb", sprintf("%05d", i), ".asc")
  index2 <- index2+1
}
...
index3 <- 1
for (i in s3) {
  file_paths3[index3] <- paste0("./30/GB_B100_30J_Y-90000_gb", sprintf("%05d", i), ".asc")
  index3 <- index3+1
}
...
index4 <- 1
for (i in s4) {
  file_paths4[index4] <- paste0("./45/GB_B100_45J_Y-90000_gb", sprintf("%05d", i), ".asc")
  index4 <- index4+1
}
...
index5 <- 1
for (i in s5) {
  file_paths5[index5] <- paste0("./60/GB_B100_60J_Y-90000_gb", sprintf("%05d", i), ".asc")
  index5 <- index5+1
}
```
...
hm1 <- ggplot(hm_data_1, mapping = aes(X, Y, fill=Z)) +
  geom_tile() +
  scale_fill_gradient2(limits = c(-0.3, 0.3), low = "black", mid = "white", high = "red", midpoint = 0) +
...
png(filename = "GB_B100J_Y_v2.png", width = 4800, height = 4800, pointsize = 1, res = 300)
